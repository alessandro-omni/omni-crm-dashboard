...
function OverviewSection({ state, presentationMode, dispatch }) {
  ...
  const fWhatsapp = useMemo(() => filterByDateRange(state.whatsappFlows, start, end, 'week'), [state.whatsappFlows, start, end]);
  const fPostcard = useMemo(() => filterByDateRange(state.postcardFlows, start, end, 'week'), [state.postcardFlows, start, end]);
  ...
  const fSubscriptions = useMemo(() => filterByDateRange(state.subscriptions, start, end, 'month'), [state.subscriptions, start, end]);
  ...
  const cWhatsapp = useMemo(() => compRange ? filterByDateRange(state.whatsappFlows, compRange.start, compRange.end, 'week') : [], [state.whatsappFlows, compRange]);
  const cPostcard = useMemo(() => compRange ? filterByDateRange(state.postcardFlows, compRange.start, compRange.end, 'week') : [], [state.postcardFlows, compRange]);
  ...
  const cSubscriptions = useMemo(() => compRange ? filterByDateRange(state.subscriptions, compRange.start, compRange.end, 'month') : [], [state.subscriptions, compRange]);

  // Aggregate CRM revenue from all channel sources (emailFlows + outreach + whatsappFlows + postcardFlows)
  const emailRevTotal = fEmailFlows.reduce((s, r) => s + (r.revenue || 0), 0);
  const outreachRevTotal = fOutreach.reduce((s, r) => s + (r.revenue || 0), 0);
  const whatsappRevTotal = fWhatsapp.reduce((s, r) => s + (r.revenue || 0), 0);
  const postcardRevTotal = fPostcard.reduce((s, r) => s + (r.revenue || 0), 0);
  const crmRevWeek = emailRevTotal + outreachRevTotal + whatsappRevTotal + postcardRevTotal;
  ...
  const latestSub = fSubscriptions.length > 0 ? fSubscriptions[fSubscriptions.length - 1] : null;
  ...
  const comp = useMemo(() => {
    ...
    const cEmailRev = cEmailFlows.reduce((s, r) => s + (r.revenue || 0), 0);
    const cOutRev = cOutreach.reduce((s, r) => s + (r.revenue || 0), 0);
    const cWaRev = cWhatsapp.reduce((s, r) => s + (r.revenue || 0), 0);
    const cPcRev = cPostcard.reduce((s, r) => s + (r.revenue || 0), 0);
    const cCrmRev = cEmailRev + cOutRev + cWaRev + cPcRev;
    const cTotalRev = cRevenue.length > 0 ? cRevenue.reduce((s, r) => s + (r.totalRevenue || 0), 0) : 1;
    ...
    const cLatestSub = cSubscriptions.length > 0 ? cSubscriptions[cSubscriptions.length - 1] : null;
    return { crmRev: cCrmRev, crmPct: cCrmPct, members: cLatestLoy?.totalMembers, atRisk: cLatestSeg?.segAtRisk, activeSubs: cLatestSub?.activeSubscribers, mrr: cLatestSub?.mrr };
  }, [compRange, cEmailFlows, cOutreach, cWhatsapp, cPostcard, cRevenue, cLoyalty, cSegments, cSubscriptions]);

  const allWeeks = [...new Set([...fEmailFlows.map(r => r.week), ...fOutreach.map(r => r.week), ...fWhatsapp.map(r => r.week), ...fPostcard.map(r => r.week)])].sort();
  // Build comparison CRM revenue trend (indexed by position, not date)
  const compCrmWeeks = compRange ? [...new Set([...cEmailFlows.map(r => r.week), ...cOutreach.map(r => r.week), ...cWhatsapp.map(r => r.week), ...cPostcard.map(r => r.week)])].sort() : [];
  const compCrmValues = compCrmWeeks.map(w => {
    ...
    const waRev = cWhatsapp.filter(r => r.week === w).reduce((s, r) => s + (r.revenue || 0), 0);
    const pcRev = cPostcard.filter(r => r.week === w).reduce((s, r) => s + (r.revenue || 0), 0);
    return eRev + oRev + waRev + pcRev;
  });
  const crmTrend = allWeeks.map((w, i) => {
    ...
    // WhatsApp: from both outreach (legacy) and whatsappFlows
    const whatsappRev = fOutreach.filter(r => r.week === w && r.channel === 'WhatsApp').reduce((s, r) => s + (r.revenue || 0), 0)
      + fWhatsapp.filter(r => r.week === w).reduce((s, r) => s + (r.revenue || 0), 0);
    // Postcards: from both outreach (legacy) and postcardFlows
    const postcardRev = fOutreach.filter(r => r.week === w && r.channel === 'Postcard').reduce((s, r) => s + (r.revenue || 0), 0)
      + fPostcard.filter(r => r.week === w).reduce((s, r) => s + (r.revenue || 0), 0);
  ...
  const monthlyChannelData = useMemo(() => {
    ...
    fWhatsapp.forEach(r => addRev(r.week, 'WhatsApp', r.revenue || 0));
    fOutreach.filter(r => r.channel === 'Postcard').forEach(r => addRev(r.week, 'Postcards', r.revenue || 0));
    fPostcard.forEach(r => addRev(r.week, 'Postcards', r.revenue || 0));
  ...
  }, [state.channelCosts, fEmailFlows, fOutreach, fWhatsapp, fPostcard]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div className="crm-kpi-grid" style={{ display: 'grid', gap: 12 }}>
        ...
        <KPICard label="Active Subscribers" value={latestSub?.activeSubscribers} format="number" status="good" sparkData={fSubscriptions} sparkKey="activeSubscribers" compDelta={comp ? pctChange(latestSub?.activeSubscribers, comp.activeSubs) : null} compLabel={compLabel} presentationMode={presentationMode} />
        <KPICard label="Monthly Recurring Revenue" value={latestSub?.mrr} format="currency" status="good" sparkData={fSubscriptions} sparkKey="mrr" compDelta={comp ? pctChange(latestSub?.mrr, comp.mrr) : null} compLabel={compLabel} presentationMode={presentationMode} />
...
function EmailFlowsSection({ state, presentationMode, dispatch }) {
  ...
  const campaignNames = [...new Set(data.filter(r => r.type === 'Campaign').map(r => r.flowName))];
  const campaignPerformance = campaignNames.map(cn => {
    const rows = data.filter(r => r.flowName === cn && r.type === 'Campaign');
    const sends = rows.reduce((s, r) => s + (r.sends || 0), 0);
    ...
    return { name: cn, sends, opens, openRate: sends > 0 ? (opens / sends) * 100 : 0, clicks, ctr: sends > 0 ? (clicks / sends) * 100 : 0, revenue: rev, conversions: conv };
  }).sort((a, b) => b.revenue - a.revenue);

  // Flow comparison: reach vs engagement
  const flowComparison = flowNames.map(fn => {
    const rows = data.filter(r => r.flowName === fn && r.type === 'Flow');
    const sends = rows.reduce((s, r) => s + (r.sends || 0), 0);
    const opens = rows.reduce((s, r) => s + (r.opens || 0), 0);
    const conv = rows.reduce((s, r) => s + (r.conversions || 0), 0);
    return { flowName: fn, sends, openRate: sends > 0 ? (opens / sends) * 100 : 0, convRate: sends > 0 ? (conv / sends) * 100 : 0 };
  }).sort((a, b) => b.sends - a.sends);

  // Abandonment flow conversion rate
  const ABANDON_PATTERN = /abandon|cart|checkout/i;
  const abandonFlows = data.filter(r => r.type === 'Flow' && ABANDON_PATTERN.test(r.flowName));
  const abandonSends = abandonFlows.reduce((s, r) => s + (r.sends || 0), 0);
  const abandonConversions = abandonFlows.reduce((s, r) => s + (r.conversions || 0), 0);
  const abandonConvRate = abandonSends > 0 ? (abandonConversions / abandonSends) * 100 : 0;
  const abandonTrend = weeks.map(w => {
    const rows = abandonFlows.filter(r => r.week === w);
    const s = rows.reduce((a, r) => a + (r.sends || 0), 0);
    const c = rows.reduce((a, r) => a + (r.conversions || 0), 0);
    return { week: w.slice(5), convRate: s > 0 ? (c / s) * 100 : 0, sends: s, conversions: c };
  });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="crm-kpi-grid" style={{ display: 'grid', gap: 12 }}>
        ...
        <KPICard label="Abandonment Conv. Rate" value={abandonConvRate} format="percent" status={abandonConvRate > 5 ? 'good' : abandonConvRate > 2 ? 'warning' : 'danger'} presentationMode={presentationMode} />
      </div>
      ...
      {campaignPerformance.length > 0 && (
        <div style={{ background: C.cardBg, borderRadius: 6, padding: 20, border: `1px solid ${C.cardBorder}` }}>
          <ChartHeader title="Campaign Performance" tooltip="Revenue, engagement metrics, and subscriptions created per campaign." />
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>{['Campaign','Sends','Opens','Open Rate','Clicks','CTR','Revenue','Subs Created'].map(h => (
                  <th key={h} style={{ padding: '8px 10px', textAlign: h === 'Campaign' ? 'left' : 'right', borderBottom: `2px solid ${C.cardBorder}`, color: C.textSecondary, fontWeight: 600, fontSize: 11 }}>{h}</th>
                ))}</tr>
              </thead>
              <tbody>
                {campaignPerformance.map((r, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${C.divider}` }}>
                    <td style={{ padding: '8px 10px', fontWeight: 500, color: C.textPrimary }}>{r.name}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: C.textSecondary }}>{formatNumber(r.sends)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: C.textSecondary }}>{formatNumber(r.opens)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: C.textSecondary }}>{formatPercent(r.openRate)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: C.textSecondary }}>{formatNumber(r.clicks)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: C.textSecondary }}>{formatPercent(r.ctr)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 600, color: C.textPrimary }}>{formatCurrency(r.revenue)}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 600, color: r.conversions > 0 ? C.success : C.textTertiary }}>{formatNumber(r.conversions)}</td>
                  </tr>
                ))}
                <tr style={{ borderTop: `2px solid ${C.cardBorder}`, fontWeight: 700 }}>
                  <td style={{ padding: '8px 10px', color: C.textPrimary }}>Total</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>{formatNumber(campaignPerformance.reduce((s, r) => s + r.sends, 0))}</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>{formatNumber(campaignPerformance.reduce((s, r) => s + r.opens, 0))}</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>{(() => { const ts = campaignPerformance.reduce((s, r) => s + r.sends, 0); const to = campaignPerformance.reduce((s, r) => s + r.opens, 0); return formatPercent(ts > 0 ? (to / ts) * 100 : 0); })()}</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>{formatNumber(campaignPerformance.reduce((s, r) => s + r.clicks, 0))}</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>{(() => { const ts = campaignPerformance.reduce((s, r) => s + r.sends, 0); const tc = campaignPerformance.reduce((s, r) => s + r.clicks, 0); return formatPercent(ts > 0 ? (tc / ts) * 100 : 0); })()}</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>{formatCurrency(campaignPerformance.reduce((s, r) => s + r.revenue, 0))}</td>
                  <td style={{ padding: '8px 10px', textAlign: 'right', color: C.success }}>{formatNumber(campaignPerformance.reduce((s, r) => s + r.conversions, 0))}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {flowComparison.length > 0 && (
        <div style={{ background: C.cardBg, borderRadius: 6, padding: 20, border: `1px solid ${C.cardBorder}` }}>
          <ChartHeader title="Flow Comparison: Reach vs Engagement" tooltip="Compares sends (reach) against open rate and conversion rate across flows." />
          <ResponsiveContainer width="100%" height={chartHeight}>
            <ComposedChart data={flowComparison}>
              <CartesianGrid strokeDasharray="3 3" stroke={C.divider} />
              <XAxis dataKey="flowName" tick={{ fontSize: 10, fill: C.textTertiary }} interval={0} angle={-25} textAnchor="end" height={60} />
              <YAxis yAxisId="left" tick={{ fontSize: 11, fill: C.textTertiary }} tickFormatter={v => formatNumber(v)} />
              <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: C.textTertiary }} tickFormatter={v => `${v}%`} />
              <Tooltip content={<ChartTooltip formatter={(v, name) => name.includes('Rate') ? `${v.toFixed(1)}%` : formatNumber(v)} />} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Bar yAxisId="left" dataKey="sends" fill={C.info} name="Sends" radius={[4,4,0,0]} opacity={0.7} />
              <Line yAxisId="right" type="monotone" dataKey="openRate" stroke={C.primary} strokeWidth={2} dot={{ r: 4 }} name="Open Rate %" />
              <Line yAxisId="right" type="monotone" dataKey="convRate" stroke={C.success} strokeWidth={2} dot={{ r: 4 }} name="Conv. Rate %" />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      )}

      {/* Abandonment Flow Trend */}
      {abandonFlows.length > 0 && (
        <div style={{ background: C.cardBg, borderRadius: 6, padding: 20, border: `1px solid ${C.cardBorder}` }}>
          <ChartHeader title="Abandonment Flow Conversion Trend" tooltip="Weekly conversion rate for abandonment flows (Abandoned Cart, Checkout, etc.)." />
          <ResponsiveContainer width="100%" height={chartHeight}>
            <ComposedChart data={abandonTrend}>
              <CartesianGrid strokeDasharray="3 3" stroke={C.divider} />
              <XAxis dataKey="week" tick={{ fontSize: 11, fill: C.textTertiary }} />
              <YAxis yAxisId="left" tick={{ fontSize: 11, fill: C.textTertiary }} tickFormatter={v => `${v}%`} />
              <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: C.textTertiary }} />
              <Tooltip content={<ChartTooltip formatter={(v, name) => name.includes('Rate') ? `${v.toFixed(1)}%` : formatNumber(v)} />} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Bar yAxisId="right" dataKey="sends" fill={C.info} name="Sends" radius={[4,4,0,0]} opacity={0.3} />
              <Line yAxisId="left" type="monotone" dataKey="convRate" stroke="#D81F26" strokeWidth={2} dot={{ r: 4 }} name="Conv. Rate %" />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      )}
...
export default function App() {
  ...
  const loggedDispatch = useCallback((action) => {
    ...
    if (action.type === 'APPEND_DATA') {
      ...
      // Persist full array — use setTimeout to read stateRef after React has committed the dispatch
      const src = action.source || 'emailFlows';
      if (conn && DATA_KEYS.includes(src)) {
        setTimeout(() => {
          const updatedData = stateRef.current[src] || [];
          saveDashboardData(conn, src, updatedData);
        }, 100);
      }
    }
    ...
    if (action.type === 'DELETE_ROWS') {
      ...
      // Persist updated array after React commits
      if (conn && DATA_KEYS.includes(action.source)) {
        setTimeout(() => {
          saveDashboardData(conn, action.source, stateRef.current[action.source] || []);
        }, 100);
      }
    }
  }, [currentUser]);
